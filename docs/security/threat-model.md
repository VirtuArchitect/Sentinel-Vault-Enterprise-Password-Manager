...
| Service-token abuse | Scoped token retrieval, audit, last-used metadata, use counts, rotation, keyed HMAC token hashes, legacy hash migration on successful use, and DevOps token response preflight/evidence gate | Completed production pipeline compromise-response evidence |
