...
| Unapproved privileged work | Live ITSM ticket lookup rejects invalid or inactive tickets before access requests are created, and Sentinel posts redacted request IDs/status updates into ITSM work notes | Provider-specific approval-state lookup and deployment-specific work-note evidence |
