...
test("production release archive binds closure archive and release gate report", () => {
  ...
  try {
    ...
    assert.equal(archive.closure.commandCoverageSummary.signoffValidatorCommandCount, 14);
    ...
    assert.equal(archive.closure.markdownCoverageSummary.validatorCommandCount, 14);
