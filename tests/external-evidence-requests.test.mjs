...
test("external evidence request generator writes remaining phase request pack", () => {
  ...
  try {
    ...
    assert.ok(report.summary.commandScripts.includes("validate:connector-preflight"));
    ...
    assert.ok(report.requests.some((request) => request.commands.includes("pnpm validate:normalized-import -- --csv <normalized-import.csv> --adapter-evidence <migration-evidence.json> --out <normalized-import-validation.json>")));
    assert.ok(report.requests.some((request) => request.commands.includes("pnpm validate:connector-preflight -- --preflight <connector-live-preflight.json> --require-siem --require-itsm")));
