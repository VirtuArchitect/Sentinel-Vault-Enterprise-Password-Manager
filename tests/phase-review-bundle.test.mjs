...
test("phase review bundle hashes final review artifacts", () => {
  ...
  try {
    ...
    assert.equal(manifest.commandCoverageSummary.signoffValidatorCommandCount, 20);
    ...
    assert.equal(manifest.commandCoverageSummary.waiverValidatorCommandCount, 20);
    ...
    assert.equal(manifest.markdownCoverageSummary.validatorCommandCount, 20);
