...
test("phase review bundle hashes final review artifacts", () => {
  ...
  try {
    ...
    assert.equal(manifest.commandCoverageSummary.signoffValidatorCommandCount, 16);
    ...
    assert.equal(manifest.commandCoverageSummary.waiverValidatorCommandCount, 16);
    ...
    assert.equal(manifest.markdownCoverageSummary.validatorCommandCount, 16);
