...
test("phase closure archive binds review bundle to Git provenance", () => {
  ...
  try {
    ...
    assert.equal(archive.review.commandCoverageSummary.signoffValidatorCommandCount, 18);
    ...
    assert.equal(archive.review.markdownCoverageSummary.validatorCommandCount, 18);
