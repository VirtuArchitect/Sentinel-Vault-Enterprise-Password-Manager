...
test("phase closure archive binds review bundle to Git provenance", () => {
  ...
  try {
    ...
    assert.equal(archive.review.commandCoverageSummary.signoffValidatorCommandCount, 21);
    ...
    assert.equal(archive.review.markdownCoverageSummary.validatorCommandCount, 21);
