...
test("phase closure archive binds review bundle to Git provenance", () => {
  ...
  try {
    ...
    assert.equal(archive.review.commandCoverageSummary.signoffValidatorCommandCount, 16);
    ...
    assert.equal(archive.review.markdownCoverageSummary.validatorCommandCount, 16);
