...
For production bundles, `pnpm validate:deployment-evidence` also requires browser extension identity and rollout evidence to be `production`, the Phase 5 connector evidence to be `certified`, the ITSM work-note evidence to be `production`, the SIEM receiver rotation evidence to be `certified`, the Windows signing execution evidence to be `signed`, and those evidence files to match the bundle environment.
